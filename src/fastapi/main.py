...
async def create_jacklified(
    ...
    """
    for file_name in os.listdir(os.path.join(os.environ['PROJECT_ROOT'], 'src/fastapi/static')):
        file_path = os.path.join(os.path.join(os.environ['PROJECT_ROOT'], 'src/fastapi/static'), file_name)
        os.remove(file_path)

    file_name, file_extension = os.path.splitext(file.filename)
    async with aiofiles.open(os.path.join(os.environ['PROJECT_ROOT'], f'src/fastapi/static/source{file_extension}'), 'wb') as out_file:
        content = await file.read()
        picture = f'source{file_extension}'
        await out_file.write(content)

    background_tasks.add_task(jackalify,
                              os.path.join(os.environ['PROJECT_ROOT'], f'src/fastapi/static/source{file_extension}'),
                              os.path.join(os.environ['PROJECT_ROOT'], 'src/fastapi/static/jackalified.mp4'))
